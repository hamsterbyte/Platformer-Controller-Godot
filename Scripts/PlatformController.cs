...
public partial class PlatformController : CharacterBody2D{
    ...
    /// <summary>
    /// If FPS exceed physics FPS then remove set the character sprite as top level and interpolate the sprites position
    ...
    private void InterpolatePlayerPosition(){
        ...
        if (currentFPS > 60){
            ...
            _sprite.GlobalPosition = _sprite.Transform.Origin.Lerp(Transform.Origin, (float)Engine.GetPhysicsInterpolationFraction());
        }
    }
    #endregion
    ...
    private void GatherInput(){
        ...
        if (_moveInput.LengthSquared() != 0){
            if (Input.IsActionJustPressed("Dash") && !_didDash && !_isDashing && _moveInput.GetDirection().Vector.Y <= 0){
                _didDash = true;
            }

            if (Input.IsActionJustPressed("Dash") && !_didDive && !_isDiving && _moveInput.GetDirection().Vector.Y > 0 &&
                !Grounded){
                _didDive = true;
            }
        }
    ...
    /// <summary>
    /// Calculate bi-directional velocity, then apply to the character
    /// </summary>
    /// <param name="delta"></param>
